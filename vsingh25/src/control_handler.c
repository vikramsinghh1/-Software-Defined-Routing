...
#include "../include/vi_globals.h"
#include "../include/vi_control_responses.h"
#include "../include/vi_router_info.h"
...
/* kept for reference, delete later
        uint8_t controlacoda;
        #define CNTRL_CONTROL_CODE_OFFSET 0x04
        int address = atoi(cntrl_header) + 4;
   //     memcpy(&controlacoda, cntrl_header+CNTRL_CONTROL_CODE_OFFSET, sizeof(controlacoda));
        //memcpy(&payload_len, cntrl_header+CNTRL_PAYLOAD_LEN_OFFSET, sizeof(payload_len));
//        printf("\n extracted from memory %d \n cntrl_header %x CNTRL_CONTROL_CODE_OFFSET %x \n",controlacoda,cntrl_header,CNTRL_CONTROL_CODE_OFFSET);
        uint16_t ploada;
        memcpy(&controlacoda, cntrl_header+4, sizeof(controlacoda));
        memcpy(&ploada, cntrl_header+6, sizeof(ploada));
        printf("\n extracted from memory control code : %d  payload length: %d \n",controlacoda,ntohs(ploada));
*/
    #endif
    #ifndef PACKET_USING_STRUCT
